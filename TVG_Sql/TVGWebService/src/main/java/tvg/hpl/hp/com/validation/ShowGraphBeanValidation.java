...
public class ShowGraphBeanValidation extends Exception {
	private static final long serialVersionUID = 1L;

	public ShowGraphBeanValidation(Set<ConstraintViolation<ShowGraphBean>> constraintViolations) {
		ObjectMapper mapper = new ObjectMapper();
		ObjectNode objectNode = mapper.createObjectNode();
		objectNode.put("status code", ApplicationConstants.RESPONSE_CODE_BAD_REQUEST);
		for (ConstraintViolation<ShowGraphBean> cons : constraintViolations) {
