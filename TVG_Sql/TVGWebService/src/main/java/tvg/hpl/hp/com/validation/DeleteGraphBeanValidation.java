...
public class DeleteGraphBeanValidation extends Exception {
	private static final long serialVersionUID = 1L;

	public DeleteGraphBeanValidation(Set<ConstraintViolation<DeleteGraphBean>> constraintViolations) {
		ObjectMapper mapper = new ObjectMapper();
		ObjectNode objectNode = mapper.createObjectNode();
		objectNode.put("status code",  ApplicationConstants.RESPONSE_CODE_BAD_REQUEST);
		for (ConstraintViolation<DeleteGraphBean> cons : constraintViolations) {
